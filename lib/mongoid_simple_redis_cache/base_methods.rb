module MongoidSimpleRedisCache
  module BaseMethods
    ...
    def run_redis_cache_by_callback_type(callback_type)
      Management.run_all_logic_by_rules(self, callback_type)
    rescue Exception => ex
      p ex.message
    ...
    def respond_to_with_redis_cache_rule?(method_id, include_all=false)
      if respond_to_without_redis_cache_rule?(method_id, include_all)
        ...
        return Management.has_method?(self,method_id.to_sym)
      end
    end

    def method_missing_with_redis_cache_rule(method_id, *args)
      if Management.has_method?(self, method_id)
        return Management.do_method(self, method_id, *args)
      else
